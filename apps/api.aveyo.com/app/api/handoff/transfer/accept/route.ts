...
import { createTransferAcceptResult, type TransferDecisionBody } from "@/lib/handoff/service";
...
export async function POST(request: Request) {
  try {
    ...
    const timedResult = await runPerfRoute(
      request,
      "api.handoff.transfer.accept",
      () => createTransferAcceptResult(body, auth.user.id),
      {
    ...
    return perfErrorJson(
      { error: "Unable to accept transfer." },
      { status: 500 },
      perfSnapshot
    );
  }
