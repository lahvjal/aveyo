...
import { createTransferCancelResult, type TransferDecisionBody } from "@/lib/handoff/service";
...
export async function POST(request: Request) {
  try {
    ...
    const timedResult = await runPerfRoute(
      request,
      "api.handoff.transfer.cancel",
      () => createTransferCancelResult(body, auth.user.id),
      {
