...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(await getManagerHandoffsResult(range, auth.user.id, auth.role));
  } catch (error) {
    if (error instanceof ServiceError) {
      return NextResponse.json({ error: error.message }, { status: error.status });
    }
    return NextResponse.json({ error: "Unable to load manager handoff activity." }, { status: 500 });
  }
}
