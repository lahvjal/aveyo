-- Ava chat policy: support-agent names shown to customers must be first name only.
-- Updates the handoff claim/resolve RPCs so system messages ("Connected with X!",
-- "Disconnected from X", "Rate your experience with X") only ever embed the
-- agent's first name.
...
DECLARE
  ...
  v_representative_first_name TEXT;
BEGIN
  IF p_request_id IS NULL OR p_actor_user_id IS NULL THEN
    RETURN;
  END IF;

  v_representative_first_name := COALESCE(
    NULLIF(split_part(trim(p_representative_name), ' ', 1), ''),
    'Representative'
  );
  ...
  VALUES (
    ...
    'system',
    p_actor_user_id,
    format('Connected with %s!', v_representative_first_name),
    jsonb_build_object(
    ...
    (
      ...
      jsonb_build_object(
        'representative', jsonb_build_object(
          'id', p_actor_user_id,
          'name', v_representative_first_name,
...
BEGIN
  ...
  IF FOUND THEN
    ...
    IF v_request.claimed_by_auth_user_id IS NOT NULL THEN
      SELECT COALESCE(
        NULLIF(split_part(trim(preferred_name), ' ', 1), ''),
