-- Reformat imported marketing news article bodies so legacy HTML line breaks become real markdown paragraphs.

UPDATE public.marketing_news_posts
  SET body_markdown = $p1body$Today is a big day for Aveyo.
...
As always, we’re excited to see how this next evolution strengthens our teams and accelerates our shared mission.$p1body$
WHERE slug = $p1slug$a-new-ceo$p1slug$;

UPDATE public.marketing_news_posts
  SET body_markdown = $p2body$**The Update**
...
Our new website reflects our unwavering commitment to delivering an exceptional solar experience at every touchpoint. Whether you’re just starting your solar journey here or revisiting us mid-process, we hope you feel our dedication—not just to you, our valued customer, but also to the promise of a brighter, solar-powered future.$p2body$
WHERE slug = $p2slug$a-new-site$p2slug$;

UPDATE public.marketing_news_posts
  SET body_markdown = $p3body$**WE’RE LAUNCHING AVA, OUR AI CHATBOT**
...
[Watch video](https://www.youtube.com/watch?v=Z4bAwM7C_Mo)$p3body$
WHERE slug = $p3slug$ava-our-ai-chatbot$p3slug$;

UPDATE public.marketing_news_posts
  SET body_markdown = $p4body$We feel incredibly honored, and a little overwhelmed, to be named UV 50’s #1 Startup to Watch. The feeling of seeing our name at the top of that exclusive list is still sinking in, especially when we consider the giants we share this list with: massive businesses like Breeze Airways, Purple Matress, Crumbl, and Built Bar.
...
So, thank you to all those who have helped Aveyo become what it is, and to all those who will push it forward to what it will be tomorrow.$p4body$
WHERE slug = $p4slug$aveyo-named-as-uv-50s-top-startup-to-watch$p4slug$;

UPDATE public.marketing_news_posts
  SET body_markdown = $p6body$Aveyo keeps on growing!
...
We couldn’t be more excited to see what he’ll do as COO and how his experience will shape Aveyo’s future.$p7body$
WHERE slug = $p7slug$rapid-growth-requires-a-high-quality-coo$p7slug$;

UPDATE public.marketing_news_posts
  SET body_markdown = $p8body$Many people have two, burning questions about solar:
...
[https://aveyo.com/locations](https://aveyo.com/locations)$p8body$
WHERE slug = $p8slug$solar-savings$p8slug$;

UPDATE public.marketing_news_posts
  SET body_markdown = $p9body$[Watch video](https://youtu.be/bxzR7rmzF7c)
...
Have questions? We’ve got answers. Talk to Aveyo today — we’ll help you go solar with confidence and make sure you don’t leave money on the table.$p9body$
WHERE slug = $p9slug$the-big-beautiful-bill$p9slug$;

UPDATE public.marketing_news_posts
  SET body_markdown = $p10body$Early 2023, 10 solar companies—Axis, Primo, Vantage Solar, LinkUs, Gen, TwoTwelve, Off Grid Energy, Caliber Solar, Blue Sky Energy Solutions, GR8FL Solar, Flex PWR, AP Pros, and Solfinity—merged together. Each with their individual strengths, possibilities, and assets. The leaders of these 10 solar companies saw a chance to do something bigger than themselves. To create a better name in the world of solar, all while providing consumers and reps with a much better experience.
...
WHERE slug = $p10slug$the-merger-2$p10slug$;

UPDATE public.marketing_news_posts
  SET body_markdown = $p11body$We’re used to having choices. Lots of choices.
...
Solar, as its adoption and acceptance grows, becomes the David to stand up to the energy-monopoly Goliath. As more individuals opt to go solar, these energy providers begin feeling a pressure to change their approach, make their energy production more efficient, and be more careful with price adjustments and hikes.$p11body$
WHERE slug = $p11slug$the-problem-with-the-monopoly-of-energy-companies$p11slug$;
